// isNan() : 어떤 값이 NaN(숫자가 아닌지)인지 판별함
// 숫자로 판별되는 것들은 false를 반환
// 숫자로 판별되지 않는 것들은 true를 반환
...
// isNan은 Numer로 숫자변환이 가능한 값을 false로 줌
...
let value2 = "p2o4";
let sum2 = 0;
if(typeof(parseInt(value2[0])) === "number"){
    sum2 += parseInt(value2[0]);
}
if(typeof(parseInt(value2[1])) === "number"){
    sum2 += parseInt(value2[1]);
}
if(typeof(parseInt(value2[2])) === "number"){
    sum2 += parseInt(value2[2]);
}
if(typeof(parseInt(value2[3])) === "number"){
    sum2 += parseInt(value2[3]);
}
